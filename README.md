...
#### Part 3: Render the code in the browser

Lastly. React calls the ```ReactDOM.createRoot().render()``` to render the actual DOM in the browser and display the UI. This includes every behavior for each component. And transform it to a bundle of compressed and optimized javascript code (_which is usually hard to understand for us_) that the browser can understand.

Below are sample JavaScript code generated by React from the components:

| Command | Description | Description |
| --- | --- | --- |
| ![Document Object Model](./docs/DOM.png) | ![H2: Hello React](./docs/HelloReact.png) | ![Welcome to this page!](./docs/WelcomeDesc.png) |

> **Note**
> With React, we can make dev-friendly syntax to create the behavior of the components. And its React's job to do the complicated things in javascript.
>
> You can find the bundled code it in the ```Sources``` tab of your browser's developer tools.